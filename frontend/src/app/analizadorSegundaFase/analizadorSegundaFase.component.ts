...
import { AnalizadorSegundaFaseService } from '../services/analizadorSegundaFase.service';
import { ViewChild } from '@angular/core';

@Component({
  selector: 'app-analizador',
  templateUrl: './analizadorSegundaFase.component.html',
  styleUrls: ['./analizadorSegundaFase.component.css']
})
export class AnalizadorSegundaFaseComponent implements OnInit {
  ...
  constructor(private monacoLoaderService: MonacoEditorLoaderService, private analizarService: AnalizadorSegundaFaseService) {
  ...
  tabla(){
    this.mostrarSimbolo = true
    this.analizarService.Tabla().subscribe((res)=>{
      this.tablaS = res['valores']
      var aux: Array<any> = []
      for(var x =0; x< this.tablaS.length; x = x+1){
        var efe = this.tablaS[x]
        var a2 = {
          'id': efe[0],
          'valor': efe[1],
          'tipo': efe[2],
